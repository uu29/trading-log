...
const CalendarDateArea = ({ year, month, date, day, calData }: CalendarDateAreaProps) => {
  return (
    <DateArea data-set-date={date} data-set-year={year} data-set-month={month} day={day}>
      <div className={item__number}>
        {date === 1 && `${month}/`}
        {date}
      </div>
      {calData.map((d, i) => (
        <Label key={d.d_time.toString()} bgColor={colors[i]}>
          {d.title}
        </Label>
      ))}
    </DateArea>
